<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=200&section=header&text=Hooman%20Esteki&fontSize=52&fontColor=ffffff&fontAlignY=45&desc=Machine%20Learning%20Engineer%20%E2%80%A2%20Master%20of%20Data%20Science%20at%20UBC&descAlignY=64&descColor=rgba(255,255,255,0.6)&descSize=14&animation=twinkling" width="100%"/>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=14&duration=2800&pause=900&color=58A6FF&center=true&vCenter=true&width=600&height=40&lines=Building+ML+systems+that+work+in+production;AI+agents+that+ask+before+they+act+%F0%9F%9B%A1%EF%B8%8F;Self-healing+pipelines+%C2%B7+drift-aware+%C2%B7+safe;Statistics+%C2%B7+ML+%C2%B7+Production+%C2%B7+Agents)](https://git.io/typing-svg)

<br/>

[![Website](https://img.shields.io/badge/esteki.ca-0071e3?style=flat-square&logo=safari&logoColor=white)](https://www.esteki.ca)&nbsp;
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/hoomanesteki)&nbsp;
[![YouTube](https://img.shields.io/badge/YouTube-FF0000?style=flat-square&logo=youtube&logoColor=white)](https://www.youtube.com/@hoomanesteki)&nbsp;
[![Instagram](https://img.shields.io/badge/Instagram-E4405F?style=flat-square&logo=instagram&logoColor=white)](https://www.instagram.com/hoomiix)&nbsp;
![Location](https://img.shields.io/badge/Canada%2C_US_%26_Remote_(Open_to_relocate)-0d1117?style=flat-square&logo=googlemaps&logoColor=58A6FF)

</div>

<br/>

---

## 👤 Summary

ML Engineer completing a **Master of Data Science at the University of British Columbia**. 5+ years building across analytics, product, and production ML.

Focus: the gap between "promising demo" and "thing that ships." Systems that self-monitor. Agents that route correctly. Pipelines you can trust.

> 💬 Happy to connect about data 📊, AI/ML 🤖, product 🚀, or coffee ☕
> Into hiking 🥾, running 🏃, tennis 🎾, or skiing ⛷️? Even better, let's talk 😊

---

## 🔬 Featured Projects

<table width="100%">
<tr>
<td width="33%" align="center" valign="top">

<img src="https://img.shields.io/badge/PROJECT_01-0071e3?style=for-the-badge&labelColor=0d1117&color=0071e3" />

### ⚡ Real-Time ML Platform

**Self-healing. Drift-aware. Zero incidents.**

PSI statistical monitoring fires retraining the moment data shifts. New models are safety-gated before serving a single prediction. Also, no human scheduling required.

![Python](https://img.shields.io/badge/-Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![XGBoost](https://img.shields.io/badge/-XGBoost-EA4335?style=flat-square)
![MLflow](https://img.shields.io/badge/-MLflow-0194E2?style=flat-square)
![FastAPI](https://img.shields.io/badge/-FastAPI-009688?style=flat-square)
![AWS](https://img.shields.io/badge/-AWS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)

| Metric | Result |
|--------|--------|
| Drift to retrain | **< 5 min** |
| Production incidents | **0** |
| Safe model launches | **100%** |
| False positive rate | **40% to 8%** |

[![View Repo](https://img.shields.io/badge/View_Repo_%E2%86%92-0d1117?style=flat-square&logo=github&logoColor=white)](https://github.com/hoomanesteki/mlops-real-time-drift-detection)

</td>
<td width="33%" align="center" valign="top">

<img src="https://img.shields.io/badge/PROJECT_02-7c3aed?style=for-the-badge&labelColor=0d1117&color=7c3aed" />

### 🧠 OmniAgent Analytics

**Seven specialists. One orchestrator.**

200+ NLP rules route every analytics question to the right expert agent: SQL, Stats, Forecasting, Visualization, Code, Voice, or Memory.

![Python](https://img.shields.io/badge/-Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![Groq](https://img.shields.io/badge/-Groq_LLM-F55036?style=flat-square)
![MCP](https://img.shields.io/badge/-MCP_Protocol-7c3aed?style=flat-square)
![Streamlit](https://img.shields.io/badge/-Streamlit-FF4B4B?style=flat-square)
![Whisper](https://img.shields.io/badge/-Whisper-412991?style=flat-square)

| Metric | Result |
|--------|--------|
| Routing accuracy | **91%** |
| Automated tests | **195+** |
| End-to-end latency | **< 2s** |
| Security checks | **40+** |

[![View Repo](https://img.shields.io/badge/View_Repo_%E2%86%92-0d1117?style=flat-square&logo=github&logoColor=white)](https://github.com/hoomanesteki/omniagent-multi-agent-analytics)

</td>
<td width="33%" align="center" valign="top">

<img src="https://img.shields.io/badge/PROJECT_03-0d9488?style=for-the-badge&labelColor=0d1117&color=0d9488" />

### 🛡️ Symbiote Lite

**AI that asks before it acts.**

3-gate human approval workflow. SELECT-only enforced at AST level across 4 independent code layers. Full audit trail every session.

![Python](https://img.shields.io/badge/-Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![Groq](https://img.shields.io/badge/-Groq_LLM-F55036?style=flat-square)
![MCP](https://img.shields.io/badge/-MCP_Protocol-0d9488?style=flat-square)
![Gradio](https://img.shields.io/badge/-Gradio-FF7C00?style=flat-square)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square)

| Metric | Result |
|--------|--------|
| Write queries executed | **Zero** |
| Automated tests | **154+** |
| Rows validated | **1M+** |
| SQL errors pre-caught | **~15%** |

[![View Repo](https://img.shields.io/badge/View_Repo_%E2%86%92-0d1117?style=flat-square&logo=github&logoColor=white)](https://github.com/hoomanesteki/symbiote-lite-analyst-agent)

</td>
</tr>
</table>

---

## 🛠 Stack

<table width="100%">
<tr>
<th align="left">Area</th>
<th align="left">Tools & Technologies</th>
</tr>
<tr>
<td valign="top"><b>🐍 Programming & Data</b></td>
<td valign="top">

![Python](https://img.shields.io/badge/Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![NoSQL](https://img.shields.io/badge/NoSQL-47A248?style=flat-square&logo=mongodb&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Spark](https://img.shields.io/badge/Apache_Spark-E25A1C?style=flat-square&logo=apachespark&logoColor=white)

</td>
</tr>
<tr>
<td valign="top"><b>🤖 AI & Machine Learning</b></td>
<td valign="top">

![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)
![XGBoost](https://img.shields.io/badge/XGBoost-EA4335?style=flat-square)
![Groq](https://img.shields.io/badge/Groq_LLM-F55036?style=flat-square)
![MCP](https://img.shields.io/badge/MCP_Protocol-7c3aed?style=flat-square)
![Whisper](https://img.shields.io/badge/Whisper_STT-412991?style=flat-square&logo=openai&logoColor=white)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![Gradio](https://img.shields.io/badge/Gradio-FF7C00?style=flat-square)

`Deep Learning` `NLP & Transformers` `AI Agents` `Feature Engineering` `Supervised & Unsupervised Learning`

</td>
</tr>
<tr>
<td valign="top"><b>📐 Statistics</b></td>
<td valign="top">

![R](https://img.shields.io/badge/R-276DC3?style=flat-square&logo=r&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=flat-square&logo=python&logoColor=ffdd54)
![Plotly](https://img.shields.io/badge/Plotly-3F4F75?style=flat-square&logo=plotly&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?style=flat-square)

`Statistical Inference (Frequentist/Bayesian)` `Regression & GLMs` `Causal Inference` `A/B Testing & Experiment Design`

</td>
</tr>
<tr>
<td valign="top"><b>☁️ Production, Cloud & MLOps</b></td>
<td valign="top">

![MLflow](https://img.shields.io/badge/MLflow-0194E2?style=flat-square&logo=mlflow&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazonaws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat-square&logo=databricks&logoColor=white)
![BigQuery](https://img.shields.io/badge/BigQuery-4285F4?style=flat-square&logo=googlebigquery&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)

`Data Pipelines` `Distributed ML` `CI/CD` `Model Deployment` `APIs`

</td>
</tr>
</table>

---

## 📊 GitHub Activity

<table width="100%">
<tr>
<td width="33%" align="center">
<img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=hoomanesteki&theme=github_dark" />
</td>
<td width="33%" align="center">
<img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=hoomanesteki&theme=github_dark" />
</td>
<td width="33%" align="center">
<img width="100%" src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=hoomanesteki&theme=github_dark" />
</td>
</tr>
</table>

<img width="100%" src="https://nirzak-streak-stats.vercel.app/?user=hoomanesteki&theme=github_dark_dimmed&hide_border=true&ring=0071e3&fire=0071e3&currStreakLabel=58A6FF&sideLabels=58A6FF&dates=8b949e" />

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=hoomanesteki&theme=react-dark&hide_border=true&area=true&area_color=0071e3&color=58A6FF&line=0071e3&point=ffffff&bg_color=0d1117" />

---

<div align="center">

```python
if decision.needs_data():
    analyst = OmniAgent()          # 7 specialists · one orchestrator
    analyst.approve_before_run()   # 3 gates · zero writes · full audit
    analyst.self_heal_on_drift()   # <5 min detection · 0 incidents
    return analyst.ship()
```

<br/>

[![esteki.ca](https://img.shields.io/badge/%F0%9F%8C%90_esteki.ca-0071e3?style=for-the-badge)](https://www.esteki.ca)&nbsp;&nbsp;
[![LinkedIn](https://img.shields.io/badge/%F0%9F%A4%9D_Let's_Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/hoomanesteki)

<br/>

<sub>⚡ Machine Learning Engineer & Data Scientist · 🎓 Master of Data Science @ UBC · 📍 Canada, US and Remote (open to relocate)</sub>

<br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer&animation=twinkling&reversal=true" width="100%"/>

</div>
